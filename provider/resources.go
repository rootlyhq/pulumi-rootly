...
func DataSourcesMap() map[string]*tfbridge.DataSourceInfo {
	p := rootly.New("pulumi")()
	dataSourcesMap := make(map[string]*tfbridge.DataSourceInfo)
	for key := range p.DataSourcesMap {
		field_key := strings.Replace(key, "rootly_", "", 1)
		res := tfbridge.DataSourceInfo{
			Tok: tfbridge.MakeDataSource(mainPkg, mainMod, fmt.Sprintf("get%s", inflect.Pascalize(field_key))),
		}
		dataSourcesMap[key] = &res
	}
	return dataSourcesMap
}

func ResourcesMap() map[string]*tfbridge.ResourceInfo {
	p := rootly.New("pulumi")()
	resourcesMap := make(map[string]*tfbridge.ResourceInfo)
	for key := range p.ResourcesMap {
		field_key := strings.Replace(key, "rootly_", "", 1)
		res := tfbridge.ResourceInfo{
			Tok: tfbridge.MakeResource(mainPkg, mainMod, inflect.Pascalize(field_key)),
